...
static int write_new_fid(struct powernow_k8_data *data, u32 fid)
{
	...
	if ((fid & INVALID_FID_MASK) || (data->currvid & INVALID_VID_MASK)) {
		pr_debug("internal error - overflow on fid write\n");
		return 1;
	}
	...
	do {
		...
		if (i++ > 100) {
			pr_debug("Hardware error - pending bit very stuck - no further pstate changes possible\n");
			return 1;
		}
	...
	if (savevid != data->currvid) {
		pr_debug("vid change on fid trans, old 0x%x, new 0x%x\n",
		       savevid, data->currvid);
		return 1;
	}

	if (fid != data->currfid) {
		pr_debug("fid trans failed, fid 0x%x, curr 0x%x\n", fid,
			data->currfid);
		return 1;
	}
...
static int write_new_vid(struct powernow_k8_data *data, u32 vid)
{
	...
	if ((data->currfid & INVALID_FID_MASK) || (vid & INVALID_VID_MASK)) {
		pr_debug("internal error - overflow on vid write\n");
		return 1;
	}
	...
	do {
		...
		if (i++ > 100) {
			pr_debug("internal error - pending bit very stuck - no further pstate changes possible\n");
			return 1;
		}
	} while (query_current_values_with_pending_wait(data));

	if (savefid != data->currfid) {
		pr_debug("fid changed on vid trans, old 0x%x new 0x%x\n",
			savefid, data->currfid);
		return 1;
	}

	if (vid != data->currvid) {
		pr_debug("vid trans failed, vid 0x%x, curr 0x%x\n",
				vid, data->currvid);
		return 1;
	}
		...
		u32 reqfid, u32 reqvid)
{
	...
	if ((reqfid != data->currfid) || (reqvid != data->currvid)) {
		pr_debug("failed (cpu%d): req 0x%x 0x%x, curr 0x%x 0x%x\n",
				smp_processor_id(),
		...
		u32 reqvid, u32 reqfid)
{
	...
	if (savefid != data->currfid) {
		pr_debug("ph1 err, currfid changed 0x%x\n", data->currfid);
		return 1;
	}
...
static int core_frequency_transition(struct powernow_k8_data *data, u32 reqfid)
{
	...
	if (data->currfid == reqfid) {
		pr_debug("ph2 null fid transition 0x%x\n", data->currfid);
		return 0;
	}
	...
	if (data->currfid != reqfid) {
		pr_debug("ph2: mismatch, failed fid transition, curr 0x%x, req 0x%x\n",
			data->currfid, reqfid);
		return 1;
	}

	if (savevid != data->currvid) {
		pr_debug("ph2: vid changed, save 0x%x, curr 0x%x\n",
			savevid, data->currvid);
		return 1;
	}
		...
		u32 reqvid)
{
	...
	if (reqvid != data->currvid) {
		...
		if (savefid != data->currfid) {
			pr_debug("ph3: bad fid change, save 0x%x, curr 0x%x\n",
				savefid, data->currfid);
			return 1;
		}

		if (data->currvid != reqvid) {
			pr_debug("ph3: failed vid transition\n, req 0x%x, curr 0x%x",
				reqvid, data->currvid);
			return 1;
		}
	}
		...
		u8 maxvid)
{
	...
	for (j = 0; j < data->numps; j++) {
		if (pst[j].vid > LEAST_VID) {
			pr_debug(FW_BUG "vid %d invalid : 0x%x\n", j,
				pst[j].vid);
			return -EINVAL;
		}
		if (pst[j].vid < data->rvo) {
			/* vid + rvo >= 0 */
			pr_debug(FW_BUG "0 vid exceeded with pstate %d\n", j);
			return -ENODEV;
		}
		if (pst[j].vid < maxvid + data->rvo) {
			/* vid + rvo >= maxvid */
			pr_debug(FW_BUG "maxvid exceeded with pstate %d\n", j);
			return -ENODEV;
		}
		if (pst[j].fid > MAX_FID) {
			pr_debug(FW_BUG "maxfid exceeded with pstate %d\n", j);
			return -ENODEV;
		}
		if (j && (pst[j].fid < HI_FID_TABLE_BOTTOM)) {
			/* Only first fid is allowed to be in "low" range */
			pr_debug(FW_BUG "two low fids - %d : 0x%x\n", j,
				pst[j].fid);
			return -EINVAL;
		}
	...
	if (lastfid & 1) {
		pr_debug(FW_BUG "lastfid invalid\n");
		return -EINVAL;
	}
		...
		struct pst_s *pst, u8 maxvid)
{
	...
	for (j = 1; j < data->numps; j++) {
		if (pst[j-1].fid >= pst[j].fid) {
			pr_debug("PST out of sequence\n");
			return -EINVAL;
		}
	}

	if (data->numps < 2) {
		pr_debug("no p states to transition\n");
		return -ENODEV;
	}
...
static int find_psb_table(struct powernow_k8_data *data)
{
	...
	for (i = 0xc0000; i < 0xffff0; i += 0x10) {
		...
		if (psb->tableversion != PSB_VERSION_1_4) {
			pr_debug(FW_BUG "PSB table is not v1.4\n");
			return -ENODEV;
		}

		pr_debug("flags: 0x%x\n", psb->flags1);
		if (psb->flags1) {
			pr_debug(FW_BUG "unknown flags\n");
			return -ENODEV;
		}
		...
		if (cpst != 1) {
			pr_debug(FW_BUG "numpst must be 1\n");
			return -ENODEV;
		}
	 ...
	 * www.amd.com
	 */
	pr_debug(FW_BUG "No PSB or ACPI _PSS objects\n");
	pr_debug("Make sure that your BIOS is up to date and Cool'N'Quiet support is enabled in BIOS setup\n");
	return -ENODEV;
}
...
static int powernow_k8_cpu_init_acpi(struct powernow_k8_data *data)
{
	...
	if (!zalloc_cpumask_var(&data->acpi_data.shared_cpu_map, GFP_KERNEL)) {
		pr_debug("unable to alloc powernow_k8_data cpumask\n");
		ret_val = -ENOMEM;
		goto err_out_mem;
	}
...
static int get_transition_latency(struct powernow_k8_data *data)
{
	...
	if (max_latency == 0) {
		pr_debug(FW_WARN "Invalid zero transition latency\n");
		max_latency = 1;
	}
...
static long powernowk8_target_fn(void *arg)
{
	...
	if (pending_bit_stuck()) {
		pr_debug("failing targ, change pending bit set\n");
		return -EIO;
	}
	...
	if (ret) {
		pr_debug("transition frequency failed\n");
...
static void powernowk8_cpu_init_on_cpu(void *_init_on_cpu)
{
	...
	if (pending_bit_stuck()) {
		pr_debug("failing init, change pending bit set\n");
...
static int powernowk8_cpu_init(struct cpufreq_policy *pol)
{
	...
	if (!data) {
		pr_debug("unable to alloc powernow_k8_data");
		return -ENOMEM;
	}
	...
	if (powernow_k8_cpu_init_acpi(data)) {
		...
		if (num_online_cpus() != 1) {
			pr_debug_once(MISSING_PSS_MSG);
			goto err_out;
		}
		if (pol->cpu != 0) {
			pr_debug(FW_BUG "No ACPI _PSS objects for CPU other than CPU0. Complain to your BIOS vendor.\n");
			goto err_out;
		}
	...
	if (cpufreq_table_validate_and_show(pol, data->powernow_table)) {
		pr_debug(FW_BUG "invalid powernow_table\n");
